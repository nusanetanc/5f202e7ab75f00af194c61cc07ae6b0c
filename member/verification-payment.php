...
foreach ($res['payment_data'] as $payment => $pay) {
  if ($pay<>null){
$bayar = col_payment->update(array("id_user"=>$id_cust, "no"=>$pembayaran),array('$set'=>array("deskripsi"=>$pay['layanan'], "harga"=>$pay['harga'], "prorate"=>$pay['prorate'], "total_harga"=>$pay['total_harga'])));
} } }
if ($emailinvoice){
	?>
